...
from keyboards.inline.kb_for_juzs import kb_endOfJuz, kb_juz_option_to_continue
...
async def sending_list_of_juzs(call: types.callback_query, state: FSMContext):
    global lang
    print("function sending-lists-for-juzs is called")
    ...
    if call.data == "list-of-juzs":
        ...
        if lang == 'uzl':
            ...
            print("2nd condition is met")
            for key, value in surahs_and_juzs_info.parts_of_Quran_in_juzs1.items():
                value = tr.translate(text=value, src='uz', dest='en')
                msg1 += "Juz " + key + ": " + value + "\n\n"
            for key, value in surahs_and_juzs_info.parts_of_Quran_in_juzs2.items():
                value = tr.translate(text=value, src='uz', dest='en')
                msg2 += "Juz " + key + ": " + value + "\n\n"
        print("messages should be sent now")
        await call.message.answer(text=msg1)
        await call.message.answer(text=msg2, reply_markup=await going_back_to_Quran_options(lang))
    ...
    if call.data == "next-ayahs-juz":
        ...
        dig_n_juz = {
            "uzl":"Boshqa juz raqamini yuboring!",
            "uzk":"Бошқа жуз рақамини юборинг!",
            "ru":"Отправить другой номер жузи",
            "en":"Send another juz number"
        }
...
def making_msg(ayah, arabic, transliteration, translation, translation_ru, translation_en, lang):
    ...
    if lang == 'uzl':
        ...
        engName = transliterate(text=engName+" surasi", to_variant='cyrillic')+"\n"
        surahNum = transliterate(text=surahNum + "-sura", to_variant='cyrillic')+'\n'
...
async def send_ayahs(message: types.Message, state: FSMContext):
    ...
    for i in range(st_ayah - 1, lt_ayah):
        caption, audio, ayah_num = making_msg(i, arabic, transliteration, translation, translation_ru, translation_en, lang)
        if len(caption) > 4096:
    ...
    print("length: ", len(arabic))
    print("latest ayah:  ", lt_ayah)

    if lt_ayah + 10 > len(arabic):
        print("it is less than 10")
        rem_ayahs = len(arabic) - st_ayah - 9

    print("Rem ayahs are ", rem_ayahs)
    if lt_ayah == len(arabic):
        end_juz = {
            "uzl":"Juz oxiriga yetdingiz. Pastdagilardan birini tanlang!",
...
async def juz_cmd(message: types.Message, state: FSMContext):
    ...
    if num > 30 or num < 1:
        en_num_limit = {
            "uzl": "Iltimos 1 va 30 orasida bo'lgan raqam kiriting!",
            "uzk": "Илтимос 1 ва 30 орасида бўлган рақам киритинг!",
            "ru": "Пожалуйста, введите число от 1 до 30!",
