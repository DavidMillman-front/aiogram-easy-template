...
async def send_10_ayahs(call: types.callback_query.CallbackQuery, state: FSMContext):
    ...
    if call.data == "next-10-ayahs":
        ...
        await call.message.answer(main_menu[lang], reply_markup=await list_of_books(state))
        ...
        await sendAudio(call.message, state)
...
def get_json(surahNum):
    ...
    translation_ru = \
    req.get(f"http://api.alquran.cloud/v1//surah/{surahNum}/editions/ru.abuadeldata").json()['data'][0]['ayahs']
...
async def sending_ayahs(message: types.Message, state: FSMContext):
    ...
    if st_ayah == 1:
        ...
        if lang == 'ru':
            await message.answer(
                "Сура " + transliterate(str(arabic['englishName']), 'cyrillic') + "Сура " + str(arabic['number']))
        if lang == 'en':
...
async def sendAudio(message: types.Message, state: FSMContext):
    print("function started")
    async with state.proxy() as data:
        surahNum = int(data['surah-number'])
        lang = data['language']
    filename = f"{surahNum}.mp3"

    if not os.path.exists(filename):
        url = f"https://cdn.islamic.network/quran/audio-surah/128/ar.alafasy/{surahNum}.mp3"
        response = req.get(url)
        print("file is accessed")
        with open(filename, 'wb') as f:
            f.write(response.content)
        print("files was downloaded and now sending")
    else:
    ...
    print("file is sent")
    await states_for_Quran.choosing_end_surah.set()
    await message.answer("Pastdagilardan birini tanlang!", reply_markup=await end_surah(lang))
